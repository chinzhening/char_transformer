...
class ResidualLSTM(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, training=True, initial_states=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            c_h = (jnp.zeros((B, self.hidden_size)), 
                   jnp.zeros((B, self.hidden_size)))
            outputs = []
            for t in range(T):
                c_h, h_t = lstm_cell(c_h, h[:, t, :])
                outputs.append(h_t)
            ...
            if layer_idx > 0:
                h += h_residual  # Residual connection

            h = nn.LayerNorm()(h)  # Layer normalization
